    ...
    const int aad_size)
{
  assert(evp_cipher);
  assert(aad_size >= 0);
  if (!cipher_aead(evp_cipher) && aad_size != 0)
    return 0;
  return 1;
}
    ...
    const int iv_size)
{
  ...
  if (iv_size == EVP_CIPHER_iv_length(evp_cipher))
    return 1;
  return 0;
}
    ...
    const int key_size)
{
  assert(evp_cipher);
  assert(key_size >= 0);
  if (key_size == EVP_CIPHER_key_length(evp_cipher))
    return 1;
  return 0;
}
...
void task_complete(napi_env env, napi_status status, void *data)
{
  ...
  if (task->ref_key)
    OK(napi_delete_reference(env, task->ref_key));
  if (task->ref_iv)
    OK(napi_delete_reference(env, task->ref_iv));
  ...
  if (task->ref_aad)
    OK(napi_delete_reference(env, task->ref_aad));
  if (task->ref_tag)
    OK(napi_delete_reference(env, task->ref_tag));
    ...
    napi_value ref_callback)
{
  ...
  if (ref_key)
    OK(napi_create_reference(env, ref_key, 1, &task->ref_key));
  if (ref_iv)
    OK(napi_create_reference(env, ref_iv, 1, &task->ref_iv));
  OK(napi_create_reference(env, ref_source, 1, &task->ref_source));
  OK(napi_create_reference(env, ref_target, 1, &task->ref_target));
  if (ref_aad)
    OK(napi_create_reference(env, ref_aad, 1, &task->ref_aad));
  if (ref_tag)
    OK(napi_create_reference(env, ref_tag, 1, &task->ref_tag));
...
static napi_value cipher(napi_env env, napi_callback_info info)
{
  ...
  if (!cipher_valid_key_size(evp_cipher, key_size))
    THROW(env, E_KEY_INVALID);
  if (!cipher_valid_iv_size(evp_cipher, iv_size))
    THROW(env, E_IV_INVALID);
  if (!cipher_valid_aad_size(evp_cipher, aad_size))
    THROW(env, E_AAD_INVALID);
  if (!cipher_valid_tag_size(evp_cipher, tag_size))
    THROW(env, E_TAG_INVALID);
  key += key_offset;
  iv += iv_offset;
...
// We require assert() for safety (our asserts are not side-effect free):
  ...
  export_error(env, exports, "OPENSSL_VERSION", OPENSSL_VERSION_TEXT);
  return exports;
}
